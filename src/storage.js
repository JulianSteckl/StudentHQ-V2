...
const fetchServerProfile = () =>
  fetch('/api/profile', { headers: authHeaders() })
    .then(r => {
      if (r.status === 401) setGoogleAccessToken(null);
      return r.ok ? r.json() : null;
    })
    .catch(() => null);

const saveServerProfile = (profile) => {
  if (!googleAccessToken || !profile?.email) { setSyncStatus('offline'); return Promise.resolve(); }
  setSyncStatus('saving');
...
const AI_KEYS_STORAGE = 'shq-v2-ai-keys';
const clearSensitiveLocalData = () => {
  setGoogleAccessToken(null);
  try { localStorage.removeItem(AI_KEYS_STORAGE); } catch (e) {}
};

export { GOOGLE_CLIENT_ID, authHeaders, setGoogleAccessToken, restoreGoogleToken, PROFILE_KEY, loadProfile, loadProfileByEmail, saveProfile, loadUserData, saveUserData, defaultUserData, fetchServerUserData, saveServerUserData, fetchServerProfile, saveServerProfile, clearSensitiveLocalData, getSyncStatus, onSyncStatus, setSyncStatus };
